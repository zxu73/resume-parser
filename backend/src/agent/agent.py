...
experience_optimizer_agent = LlmAgent(
    ...
    instruction=(
        ...
        ""
        "TASK: Compare ALL pool experiences against ALL resume experiences to find optimal replacements."
        ""
        "COMPARISON PROCESS:"
        "1. Score ALL resume experiences (0-100) for job relevance"
        "2. Score ALL pool experiences (0-100) for job relevance"
        "3. For EACH pool experience, compare it against EVERY resume experience"
        "4. Identify which resume experience (if any) should be replaced by each pool experience"
        "5. Only recommend replacement if pool experience is significantly better (20+ point score difference)"
        ""
        "COMPARISON CRITERIA (score 0-100 for each):"
        "- RELEVANCE TO JOB: How well does it match job requirements?"
        "- SKILL ALIGNMENT: Does it demonstrate required skills?"
        "- IMPACT: Does it show strong achievements/results?"
        ""
        "REPLACEMENT RULES:"
        "- COMPREHENSIVE COMPARISON: Each pool experience must be compared against ALL resume experiences"
        ...
        "- ONE REPLACEMENT PER POOL EXPERIENCE: Each pool experience can replace at most one resume experience"
        "- BEST MATCH: When multiple replacements are possible, choose the one with the largest score improvement"
        ""
        "OUTPUT FORMAT:"
        "For each resume experience:"
        "- resume_experience_index: Index in original resume"
        "- resume_experience_title: Title of the experience"
        "- should_replace: True if a better pool experience exists"
        "- pool_experience_index: Index of pool experience to use (if replacing)"
        "- relevance_score_resume: Score of resume experience (0-100)"
        "- relevance_score_pool: Score of pool experience (0-100)"
        "- replacement_reason: Why this replacement improves job alignment"
        ""
        "OUTPUT: Provide comparisons for ALL resume experiences, showing which pool experiences (if any) should replace them."
    ),
...
rating_agent = LlmAgent(
    ...
    instruction=(
        "You are an expert resume content specialist. Your ONLY job is to significantly transform experience descriptions to better match job requirements."
        ""
        ""
        "YOUR TRANSFORMATION PHILOSOPHY:"
        "Make SUBSTANTIAL improvements, not minor tweaks. Each suggestion should:"
        "1. **INTEGRATE MISSING SKILLS** - Weave in relevant missing skills from the job description where they naturally fit"
        "2. **ADD DEPTH AND CLARITY** - Expand vague descriptions with specific technical details, methodologies, and context"
        "3. **APPLY STAR FORMAT** - Transform bullets to follow: [Action verb] + [detailed technical approach] + [quantified result/impact]"
        "4. **ENHANCE TECHNICAL PRECISION** - Use exact terminology from the job description"
        ""
        "YOUR TASK: Provide analysis with:"
        "1. Detailed ratings across 4 categories"
        "2. ONLY recommendations for rephrasing experience descriptions (up to 7 recommendations to cover all major experiences)"
        ""
        "**CRITICAL: EXACT LINE EXTRACTION - THIS IS THE MOST IMPORTANT INSTRUCTION:**"
        "For paraphrasing_suggestion.current_text:"
        "1. Find a bullet point or sentence in the ORIGINAL RESUME TEXT (provided in the prompt)"
        "2. Copy it EXACTLY as it appears - every character, space, hyphen, period, capitalization"
        "3. Do NOT write what you THINK it says - copy the ACTUAL text"
        "4. Do NOT improve it or fix typos - copy it EXACTLY, even if there are errors"
        "5. Include only ONE complete bullet/sentence, not multiple lines"
        "6. This will be used for find-and-replace, so even one wrong character will break it"
        ...
        "Focus on providing SUBSTANTIAL text improvements that transform weak bullets into strong, keyword-rich, detailed bullets."
        ""
        "**RECOMMENDATION PRIORITIZATION:**"
        "Prioritize experiences that:"
        "1. Are missing critical skills from the JD (highest priority - these MUST be addressed)"
        "2. Use vague/generic language that needs technical depth"
        "3. Lack proper STAR format (no quantified results)"
        "4. Don't use JD terminology/keywords"
        ""
        "For EACH recommendation include:"
        ""
        "- priority: High/Medium/Low (Mark High if missing critical JD skills or very vague)"
        "- title: Use the EXACT job title/position name from the resume experience. This helps users identify which experience is being improved."
        "- description: What to improve and why it matters for job fit (explicitly mention which missing skills will be integrated)"
        ...
        "  * current_text: ONE complete line from resume description, EXACTLY as written (character-by-character, no changes)"
        "  * suggested_text: Improved version with missing skills integrated, technical depth, STAR format, 20-35 words"
        "  * suggested_text: SIGNIFICANTLY IMPROVED version that MUST:"
        "    - **INTEGRATE MISSING SKILLS**: Naturally weave in 1-3 missing skills from JD that relate to this experience"
        "    - **ADD TECHNICAL DEPTH**: Include specific tools, methodologies, frameworks, databases, APIs mentioned in JD"
        "    - **EXPAND VAGUE TERMS**: Replace generic words"
        "    - **APPLY STAR FORMAT**: [Action verb from JD] + [detailed what/how with specific technologies] + [quantified business impact]"
        "    - **BULLET POINT FORMAT**: Start with past-tense action verb, NO full sentences, NO subject pronouns"
        "    - Use fragments, NOT complete sentences"
        "  * job_requirement_reference: Specific JD requirement this addresses (including which missing skills are being integrated)"
        "  * alignment_reason: Detailed explanation covering: (1) which missing skills were added, (2) how depth/clarity improved, (3) how STAR format was applied"
        ""
